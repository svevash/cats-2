...
template<typename T>
auto initialize_vector(T value, int dim) {
    return std::vector<T>(dim, value);
}

template<typename T, typename... Args>
auto initialize_vector(T value, int dim, Args&&... args) {
    auto res = initialize_vector<T>(value, std::forward<Args>(args)...);
    return std::vector<decltype(res)>(dim, res);
}
